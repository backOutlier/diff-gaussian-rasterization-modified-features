	...
	const bool include_feature)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		debug,
		include_feature ? language_feature.contiguous().data_ptr<float>() : nullptr,
		include_feature ? out_language_feature.contiguous().data_ptr<float>() : nullptr,
		include_feature);
  }
	...
	const bool include_feature) 
{
  ...
  {  
	  ...
	  debug,
	  include_feature ? language_feature.contiguous().data_ptr<float>() : nullptr,
	  include_feature ? dL_dout_language_feature.contiguous().data_ptr<float>() : nullptr,
	  include_feature ? dL_dlanguage_feature.contiguous().data_ptr<float>() : nullptr,
	  include_feature);
  }
